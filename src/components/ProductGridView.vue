<!-- src/components/ProductGridView.vue (Versión Final y Completa) -->
<script setup>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
    <div v-for="producto in productos" :key="producto.id" 
      ...
      <div class="relative overflow-hidden aspect-square">
        <!-- La imagen ahora también es un enlace a la página de detalle -->
        <RouterLink v-if="producto.slug" :to="`/producto/${producto.slug}`">
        ...
        <!-- Si no hay slug, la imagen no es un enlace -->
        <div v-else>
          <img v-if="producto.producto_imagenes && producto.producto_imagenes.length > 0" 
               :src="producto.producto_imagenes[0].imagen_url" 
               :alt="producto.nombre"
      ...
      <div class="p-6 flex-1 flex flex-col">
        <!-- El nombre del producto es un enlace condicional -->
        <RouterLink v-if="producto.slug" :to="`/producto/${producto.slug}`" class="hover:text-blue-600">
          <h2 class="text-xl font-bold text-gray-800">{{ producto.nombre }}</h2>
        </RouterLink>
        <h2 v-else class="text-xl font-bold text-gray-800">{{ producto.nombre }}</h2>
